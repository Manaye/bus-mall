...
function initializeDatabase() {
  ...
  var localImages = localStorage.getItem("my-images");
  var images = JSON.parse(localImages);
  ...
  console.log("images", Images.allImages);
}
...
function eventHandler(event){
  ...
  for( var i=0; i<Images.allImages.length; i++) {
    if(Images.allImages[i].filename === imageName ) {
      ...
      maxClicksAllowed--;      
      localStorage.setItem("my-images", JSON.stringify(Images.allImages));   
      break;
    }
  }
